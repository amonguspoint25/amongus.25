...
export function computePerf(role: "CREW" | "IMPOSTOR", s: PerfStats): number {
  if (role === "IMPOSTOR") {
    ...
    const speed = s.timeToKillMs !== undefined ? clamp((30000 - s.timeToKillMs) / 30000) * 0.3 : 0;
  ...
  const speed = s.timeToTaskMs !== undefined ? clamp((120000 - s.timeToTaskMs) / 120000) * 0.2 : 0;
